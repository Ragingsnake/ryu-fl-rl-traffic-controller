...
class NSFNETRoutingEnv(gym.Env):
    ...
    def __init__(self, topo_path: str, tm_path: str, fl_model_path: Optional[str] = None):
        super(NSFNETRoutingEnv, self).__init__()
        ...
        self.edges = sorted(list(self.topology.edges()))
    ...
    def enable_link(self, u: int, v: int, capacity: float = None, delay: float = None):
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, dict]:
        ...
        capacities = np.array([self.topology[e[0]][e[1]]["capacity"] if e not in self.disabled_links else 1.0 for e in self.edges], dtype=np.float32)
        ...
        for idx, e in enumerate(self.edges):
            ...
            # Queuing delay (M/M/1)
            if u_e < 0.95:
                # d_queue = u_e / (C * (1 - u_e)), C in bps -> convert to ms
                q_delay = (u_e / (cap_e * (1.0 - u_e))) * 1e3
            else:
                q_delay = MAX_QUEUE_DELAY_MS
...
if __name__ == "__main__":
    ...
    edges = [(0,1), (0,2), (0,7), (1,2), (1,3), (2,5), (3,4), (3,8), (4,5), (4,6),
             (5,10), (5,13), (6,7), (7,9), (8,9), (8,11), (8,12), (9,10), (10,11),
             (10,12), (11,13), (12,13)]
    mock_topo = {
        "nodes": [{"id": i} for i in range(14)],
